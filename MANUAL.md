...
| Perfil | Quando usar | Modo recomendado |
|---|---|---|
| `portrait-core` | Retrato geral | `balanced` |
| `professional-headshot` | Headshot profissional | `strict` |
| `virtual-model` | Modelo virtual consistente | `balanced` |
| `dataset-balanced` | Dataset com cobertura uniforme | `dataset` |
| `editorial-fashion` | Moda editorial | `balanced` |
| `lifestyle` | Retrato cotidiano | `balanced` |
| `street-portrait` | Retrato urbano | `balanced` |
| `studio-beauty` | Beleza de estúdio | `balanced` |
| `cinematic-portrait` | Retrato cinematográfico | `balanced` |
| `fine-art-portrait` | Retrato autoral | `balanced` |
| `historical-portrait` | Retrato histórico coerente | `strict` |
| `fantasy-portrait` | Fantasia original | `creative` |
| `dark-fantasy-horror` | Horror atmosférico não gráfico | `creative` |
| `conceptual-portrait` | Retrato conceitual | `creative` |
| `full-body-fashion` | Moda de corpo inteiro | `balanced` |
...
Seleciona um dos 15 perfis Catalog V2 listados acima. Cada perfil controla também os packs
habilitados, classes de segurança, verbosidade, densidade e módulos negativos.
...
| Modo | Efeito atual |
|---|---|
| `strict` | Remove opções experimentais, reforça preferências e nunca relaxa filtros. |
| `balanced` | Escolhe primeiro a família semântica e depois a opção ponderada; pode relaxar apenas filtros opcionais. |
| `creative` | Eleva deterministicamente a chance relativa de itens raros sem relaxar regras absolutas. |
| `dataset` | Seleciona famílias e opções de forma uniforme para maximizar cobertura. |
| `sequential` | Percorre famílias e opções em ciclo estável controlado por `batch_index`. |
...
- O schema pode ser `1.0` para perfil legado ou `2.0` para perfil Catalog V2.
...
```json
{
  "subject-type": "identity-subject-type-base-adult-woman-subtle-intensity",
  "wardrobe-theme": "wardrobe-theme-contemporary-smart-casual-precise-tailoring",
  "location": "scene-location-studio-neutral-portrait-studio-open-spatial-layout"
}
```
...
Exemplo no campo `wardrobe-theme`:
...
grupos como `identity`, `appearance`, `wardrobe`, `performance`, `scene`, `lighting`, `camera`,
`style` e `negative`.
...
- opção, pack, família, facets, variante e origem de cada seção;
...
```json
{
  ...
  "fields": {
    "subject-type": {
      ...
      "include_tags": [],
      "mode": "fixed",
      "value": "identity-subject-type-base-adult-woman-subtle-intensity"
    }
  },
  "groups": {
    "identity": {
...
O Catalog V2 possui 69 dimensões atômicas. Cada perfil mostra somente as dimensões úteis para sua
finalidade.

| Grupo | Campos |
|---|---|
| Identidade | `subject-type`, `adult-age`, `identity-archetype`, `face-shape`, `facial-structure`, `eye-color`, `eye-shape`, `eyebrows`, `nose`, `mouth`, `skin-tone`, `skin-texture`, `skin-details`, `distinguishing-features` |
| Cabelo e corpo | `hair-color`, `hair-length`, `hair-texture`, `hair-style`, `facial-hair`, `body-build` |
| Vestuário | `wardrobe-theme`, `tops`, `bottoms`, `one-piece`, `outerwear`, `footwear`, `materials`, `patterns`, `accessories`, `jewelry`, `headwear`, `palette` |
| Performance | `expression`, `gaze`, `head-position`, `pose`, `hand-pose`, `action` |
| Cena | `location`, `background`, `architecture`, `set-dressing`, `time-of-day`, `season`, `weather`, `atmosphere` |
| Luz | `lighting-setup`, `light-source`, `light-direction`, `light-quality`, `light-temperature` |
| Câmera | `shot-size`, `camera-angle`, `lens-focal`, `aperture`, `focus`, `camera-motion`, `composition`, `exposure-film-look` |
| Estilo | `photo-genre`, `visual-medium`, `visual-era`, `color-grading`, `finish`, `quality` |
| Negativo | `negative-base`, `negative-anatomy`, `negative-camera`, `negative-context` |

Os negativos são modulares: `minimal` usa somente a base, `standard` acrescenta anatomia e câmera,
e `strict` acrescenta coerência contextual. O perfil define o nível.
...
Prompt negativo final. Todos os perfis oficiais exigem conteúdo negativo conforme o nível
declarado.
...
3. Deixe `wardrobe`, `performance`, `scene`, `lighting` e `camera` derivados.
...
{
  "wardrobe-theme": "wardrobe-theme-contemporary-smart-casual-precise-tailoring",
  "location": "scene-location-urban-quiet-city-street-open-spatial-layout"
}
```
...
1. Abra **Fields**.
2. Localize o campo específico, por exemplo `wardrobe-theme`, `pose` ou `location`.
